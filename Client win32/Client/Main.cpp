...
void SDL_RenderFillRoundyRect(SDL_Renderer * rndrr, SDL_Rect * box, int r){//copied SDLs format for easy switching //precompute corner bitmaps for each call if optimization is needed
...
void SDL_RenderFillLeftRoundyRect(SDL_Renderer * rndrr, SDL_Rect * box, int r){//copied SDLs format for easy switching
    SDL_RenderFillRect(rndrr, box);
//Xiaolin Wu
    float y_c;
    unsigned char c_r, c_g, c_b, c_a;
...
void SDL_RenderFillRightRoundyRect(SDL_Renderer * rndrr, SDL_Rect * box, int r){//copied SDLs format for easy switching
    SDL_RenderFillRect(rndrr, box);
    //Xiaolin Wu
    float y_c;
    unsigned char c_r, c_g, c_b, c_a;
...
void typeIntoSelectedTextbox(char * string){
    ...
    int max_cpy = member_sizeof(textbox, string) - cursor - SDL_strlen(string)-1;//the distance from the cursor + new text to the end of the box
...
bool doSlider(char * name, int * value, int max_value, int width, int height){
    ...
    if (selected_slider == value){
        ...
        if (xnum == 0){ sprintf(val, "0"); }
        else if (xnum*xden == 1){ sprintf(val, " \xCF\x80 "); }
        else if (xnum*xden == -1){ sprintf(val, " -\xCF\x80 "); }
        else if (xnum == 1){ sprintf(val, " \xCF\x80/%i ", xden); }
        else if (xnum == -1){ sprintf(val, " -\xCF\x80/%i ", xden); }
        else if (xden == 1){ sprintf(val, " %i\xCF\x80 ", xnum); }
        else { sprintf(val, " %i\xCF\x80/%i ", xnum, xden); }//general case
...
bool doGrid(grid * g, int x_cells, int y_cells, int width, int height){
    ...
    if (over){
        ...
        //format all the special cases {except denominator = -1 since it's always positive}
        char x_coord[50];
        if (xnum == 0){sprintf(x_coord, "0");}
        else if (xnum*xden == 1){sprintf(x_coord, "\xCF\x80");}
        else if (xnum*xden == -1){sprintf(x_coord, "-\xCF\x80");}
        else if (xnum == 1){sprintf(x_coord, "\xCF\x80/%i", xden);}
        else if (xnum == -1){sprintf(x_coord, "-\xCF\x80/%i", xden);}
        else if (xden == 1){sprintf(x_coord, "%i\xCF\x80", xnum);}
        else {sprintf(x_coord, "%i\xCF\x80/%i", xnum, xden);}//general case
        char y_coord[50];
        if (ynum == 0){sprintf(y_coord, "0");}
        else if (ynum*yden == 1){sprintf(y_coord, "\xCF\x80");}
        else if (ynum*yden == -1){sprintf(y_coord, "-\xCF\x80");}
        else if (ynum == 1){sprintf(y_coord, "\xCF\x80/%i", yden);}
        else if (ynum == -1){sprintf(y_coord, "-\xCF\x80/%i", yden);}
        else if (yden == 1){sprintf(y_coord, "%i\xCF\x80", ynum);}
        else {sprintf(y_coord, "%i\xCF\x80/%i", ynum, yden);}//general case

        sprintf(coord, " (%s, %s) ", x_coord, y_coord);
...
static int connect(void * connect_data_pointer){
    connect_inout * connect_data = (connect_inout *) connect_data_pointer;
    ...
    int error = getaddrinfo(connect_data->ipa, "5206", &hints, &address_info);
    if (error != 0)
    {
        sprintf(message, "Error connecting to %s: getaddrinfo failed: %d", connect_data->ipa, error);
    }

    if(address_info == 0)
    {
        sprintf(message, "Could not resolve host name");
    }
    else{
                
        for(; address_info != 0; address_info = address_info->ai_next)
        {
            (*connect_data->sock) = INVALID_SOCKET;

            (*connect_data->sock) = socket(address_info->ai_family, address_info->ai_socktype, address_info->ai_protocol);

            if ((*connect_data->sock) == INVALID_SOCKET) {
                sprintf(message, "Socket failed with error: %d", WSAGetLastError());
                freeaddrinfo(address_info);
                return 0;
            }
            
            connect((*connect_data->sock), address_info->ai_addr, (int)address_info->ai_addrlen);
            if (error == SOCKET_ERROR) {
                closesocket(*connect_data->sock);
                (*connect_data->sock) = INVALID_SOCKET;
                continue;
            }
            break;
        ...
        if((*connect_data->sock) == INVALID_SOCKET)
        {
            sprintf(message, "Unable to connect to %s", connect_data->ipa);
            return 0;
        }
        ...
        do
        {
            n_bytes = recv(*connect_data->sock, recieve_buffer, sizeof(recieve_buffer), 0);

            if (n_bytes > 0)
            {
                sprintf(message, "Connected to %s", (connect_data)->ipa);
                (connect_data)->data = recieve_buffer[n_bytes-1];
            }
            else if (n_bytes == 0)
            {
                sprintf(message, "Connection closed by server");
            }
            else
            {
    ...
    if (SDLNet_ResolveHost(ns->ipa, ns->host, 5204) == -1){
        sprintf(message, "Error connecting to %s: %s", ns->host, SDLNet_GetError());
        return 0;
    }
    else{
        ...
        sprintf(message, "Connected to %s", ns->host);
...
int main(int argc, char* argv[]){
    ...
    connect_inout * connect_data = (connect_inout *) malloc(sizeof(connect_inout));
    ...
    while (1){
        ...
        if (expanded){
            ...
            if (ip_entered && ip_box.string[0]){
                sprintf(message, "Connecting to %s ...", ip_box.string);

                connect_data->sock = &connect_socket;
                connect_data->ipa = ip_box.string;
                connect_data->data = (g.x << 4) | (g.y & 0x0F);
                
                netthread = SDL_CreateThread(connect, "NetThread", (void *) connect_data);
        ...
        if (connect_socket != INVALID_SOCKET){
            g.x = connect_data->data >> 4;
            g.y = connect_data->data & 0x0F;
        }
        else{
            message_time = 0;
        }
        ...
        case 0:
            ...
            if (doGrid(&g, 16, 16, min(window_width, window_height) - 20, min(window_width, window_height) - 20) && connect_socket != INVALID_SOCKET)
            {
                connect_data->data = (g.x << 4) | (g.y & 0x0F);
                if (send(connect_socket, (char *)&connect_data->data, 1, 0) <= 0){
                    sprintf(message, "Not Connected: %d", WSAGetLastError());
        ...
        case 1:
        {
            x_pos = 10;
            y_pos = 10;
            bool do_send = doSlider("Longitude", &g.x, 15, window_width - 20, (window_height - 30)/2);
            y_pos += (window_height - 30) / 2+10;
            do_send = doSlider("Latitude", &g.y, 15, window_width - 20, (window_height-30)/2) || do_send;
            if (do_send && connect_socket != INVALID_SOCKET)
            {
                connect_data->data = (g.x << 4) | (g.y & 0x0F);
                if (send(connect_socket, (char *)&connect_data->data, 1, 0) <= 0)
                {
